...
```bash
sudo wsproxy test
sudo wsproxy test server --config /etc/wsproxy/server.yaml
sudo wsproxy test client --config /etc/wsproxy/client.yaml
```
...
都是「服务器上开门，流量从客户端那边出去」。可以写多条 `--expose`。安装客户端时一条一条填，写法相同。
...
| 类型 | 示例 | 含义 |
| --- | --- | --- |
| TCP | `9000=127.0.0.1:80` | 服务器本机 9000 → 客户端 80 |
| TCP | `tcp://9000=127.0.0.1:80` | 同上，写明是 TCP |
| TCP | `0.0.0.0:9000=127.0.0.1:80` | 服务器对公网开 9000 → 客户端 80 |
| TCP | `tcp://0.0.0.0:9000=10.0.0.8:22` | 对公网开，转到客户端能到的地址 |
| UDP | `udp://5353=127.0.0.1:53` | 服务器本机 UDP 5353 → 客户端 53 |
| UDP | `udp://0.0.0.0:5353=1.1.1.1:53` | 对公网开 UDP |
| SOCKS5 | `socks://1080` | 服务器本机 SOCKS，目标由连的人指定 |
| SOCKS5 | `socks5://1080` | 同上 |
| SOCKS5 | `socks://0.0.0.0:1080` | 对公网开 SOCKS |
| HTTP 代理 | `http://3128` | 服务器本机 HTTP 代理 |
| HTTP 代理 | `http://127.0.0.1:3128` | 同上，写明只听本机 |
| HTTP 代理 | `http://0.0.0.0:3128` | 对公网开 HTTP 代理 |
